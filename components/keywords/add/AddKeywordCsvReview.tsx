...
function ItemStatus({ item }: Readonly<{ item: CsvKeywordReviewItem }>) {
  if (item.issues[0]) {
    return (
      <span className="ml-auto flex-none rounded-full bg-bg-sunken px-2 py-0.5 font-mono text-[10.5px] text-red">
  ...
  if (item.alreadyTracked) {
    return (
      <span className="ml-auto flex-none rounded-full bg-bg-sunken px-2 py-0.5 font-mono text-[10.5px] text-yellow">
    ...
    <div className="grid gap-4">
      <div className="rounded-[11px] border border-border bg-bg-sunken px-4 py-3">
        <div className="flex items-start gap-3">
          <span className="mt-0.5 grid h-8 w-8 flex-none place-items-center rounded-[9px] bg-accent-soft text-accent">
      ...
      <div className="overflow-hidden rounded-[11px] border border-border">
        {preview.map((item) => (
          <div
            ...
            key={item.key}
          >
            <span className="w-6 flex-none font-mono text-[11px] text-fg-faint">
              {item.position}
            </span>
        ...
        {remaining > 0 ? (
          <div className="border-t border-border-soft px-3.5 py-2.5 font-mono text-[11px] text-fg-faint">
            +{remaining} more
          </div>
        ) : null}
      </div>
      <button
        className="inline-flex w-max items-center gap-1.5 rounded-[9px] border border-border-strong bg-bg-elev px-3 py-2 text-[12.5px] font-semibold text-fg-muted"
        onClick={onEdit}
        type="button"
      >

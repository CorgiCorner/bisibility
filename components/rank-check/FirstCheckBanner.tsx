...
export function FirstCheckBannerLink({ href, label }: Readonly<{ href: string; label: string }>) {
  return (
    <Link
      className="inline-flex shrink-0 items-center justify-center gap-1.5 rounded-lg bg-accent px-[13px] py-2 text-[12.5px] font-semibold text-white outline-none transition-colors hover:bg-accent-hover focus-visible:bg-accent-hover"
      href={href}
      {...docsLinkProps(href)}
    >
      {label}

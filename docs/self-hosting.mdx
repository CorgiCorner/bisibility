---
title: "Self-hosting"
description: "Run bisibility with Docker, Railway, Fly.io, Vercel, or your own Node.js host."
---
...
The default Docker stack starts PostgreSQL, Redis/Valkey, a one-shot
...
traffic admission. The compatibility `/api/v1/health` endpoint remains the full
diagnostic report and can return `503` for worker or Temporal failures that do
not stop the web process from serving reads. The optional `APP_REVISION`
...
v0.2.0 stops exposing PostgreSQL on `5432` and Redis/Valkey on `6379` to the
...
If the Docker host must reach PostgreSQL or Redis/Valkey, use the debug overlay
...
> publishes PostgreSQL `5432` and Redis/Valkey `6379` on all host interfaces by
...
The default stack publishes only the app on `3000`. PostgreSQL and Redis/Valkey
...
For deliberate PostgreSQL or Redis/Valkey access from the Docker host, apply the
...
The override binds PostgreSQL to `127.0.0.1:5432` and Redis/Valkey to
...
5. The web manifest runs `npm run db:migrate` as a Fly release command. It
   applies the full Prisma tree, runs active deploy-blocking data migrations, and checks
   `/api/v1/health` before routing traffic.
   Fly runs this command in a temporary machine without attached volumes and
   stops the deployment on a nonzero exit. The manifest extends Fly's default
   five-minute release-command timeout to 30 minutes for deploy-blocking migrations.
...
5. Keep preview and build jobs migration-free. Do not add a `vercel.json`
   `buildCommand` that runs database migrations.

For production, use a staged operations flow from a trusted environment:

```bash
vercel --prod --skip-domain
npm run db:migrate
curl --fail https://staged.example.com/api/v1/health
vercel promote https://staged.example.com
```

Set the production `DIRECT_URL` and `DATABASE_URL` only for the separate
`npm run db:migrate` step. Replace `https://staged.example.com` with the
unaliased deployment URL printed by the first command. Promote only after the
migration and staged smoke check succeed. Vercel preview and build execution
...
| Temporal Cloud | Managed by Temporal. Set `TEMPORAL_ADDRESS` and `TEMPORAL_API_KEY`. | Per-service platforms such as Railway or Fly.io |

On per-service platforms, Temporal Cloud removes three containers from the
deployment and leaves the web app, the worker, and the databases.

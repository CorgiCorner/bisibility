...
Cloud users do not own or operate the managed database. The ownership promise
on Cloud is exit rights: [full-history export](/api/rank-history) and the
[migration path](/guides/migration) to a self-hosted instance keep workspace
data portable.
...
Valkey, and Temporal can host bisibility. Railway is the supported bundled
one-click path; guides for other platforms will return after their deployment
and upgrade paths have been verified.
...
[Deploy bisibility on Railway](https://railway.com/deploy/bisibility).

Railway is the bundled one-click path. Its template runs the web app, worker,
PostgreSQL, Valkey, Temporal Server, and Temporal persistence inside one Railway
project, with no Temporal Cloud account. The listing is pinned to a tested
bisibility release and uses private Railway networking for every internal
connection.
...
Railway generates the database passwords, auth secret, provider-credential
encryption key, internal probe token, and deployment suffix. Reference variables
share each generated value with the services that need it; do not replace those
references with separately generated values. In particular, web and worker must
share `BISIBILITY_SECRETS_KEY`, the Temporal namespace, and both task queues.

You also need a SERP provider account, but you connect it from **Integrations**
after the application is running. The template does not include provider usage,
credits, or a provider subscription.

<Warning>
Review [Railway pricing](https://docs.railway.com/pricing) and configure an
appropriate [usage alert or hard limit](https://docs.railway.com/pricing/cost-control)
before deploying. Six services keep running, the application PostgreSQL,
Temporal PostgreSQL, and Redis volumes retain data, and two bootstrap jobs
stop after they complete. Deleting or redeploying a completed job is not a way
to reduce the cost of the six persistent services. Railway estimates monthly
usage from observed runtime consumption, not on the template review screen.
</Warning>

### What the template creates

Eight resources are expected. `Running` is normal for the six persistent roles,
and `Completed` is the successful terminal state for the two one-shot jobs.
...
1. Open the [public Railway template](https://railway.com/deploy/bisibility)
...
does not protect the other. Work through the page-wide
[production checklist](#production-checklist) before inviting users.

...
Read the [FAQ](https://bisibility.com/faq) for a concise comparison of the hosted
beta and self-hosting, provider coverage, costs, and data boundaries.

Hosted-service users do not own or operate the managed database. The ownership promise
on the hosted service is exit rights: [full-history export](/api/rank-history) and the
[migration path](/guides/migration) to a self-hosted instance keep project
data portable.
...
payloads unless you opt in. The hosted service defaults to 90 days.

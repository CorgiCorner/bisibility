...
[demo login](/quickstart#demo-login) enabled by the two `DEMO_*` variables
above. For real deployments, remove both variables and configure an
[email provider](/guides/email) instead.
...
and then any blocking data migrations. No separate operator migration job is
required. The Node server, Temporal worker, and `/api/v1/health` require every blocking data
migration to have a finished row with its exact manifest checksum. When
`DEMO_FIXED_OTP=1`, the migration service also seeds demo data for the local
dashboard.

> **Known limitation:** `services.migrations: "ready"` answers whether blocking
> data migrations have finished with their expected checksums and whether the
> public ID database contract is ready. It does not report whether the Prisma
> schema is current. Run `npx prisma migrate status` in the deployed application
> environment to check schema currency. This is unchanged from 0.1.0 and is
...
5. The web manifest runs `npm run db:migrate` as a Fly release command. It
   applies the full Prisma tree, runs blocking data migrations, and checks
   ...
   five-minute release-command timeout to 30 minutes for blocking migrations.

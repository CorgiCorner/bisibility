...
There are two Docker paths. The local demo builds from a repository checkout.
...
Both paths start the same application services. The production asset contains
no `build:` entries; the local checkout keeps them for source development.
...
the migration job and app. The production asset pins the exact release and
cannot build from a checkout. The repository Compose file retains
`BISIBILITY_IMAGE`, `BISIBILITY_WORKER_IMAGE`, and `--build` for source testing.
...
That profile adds Temporal Server 1.31.2, Temporal Web UI on
`http://localhost:8233`, and the version-pinned worker image from Docker Hub.
An idempotent Admin Tools job creates or upgrades both Temporal schemas and the
namespace before the worker starts. Set
`BISIBILITY_WORKER_IMAGE` to another complete image reference, or add `--build`
to build the web and worker images from the checkout.
...
For scheduled checks, start the same versioned asset with its scheduled profile:
...
The release manifest records SHA-256 hashes for the Compose, environment, and
generator assets. The public release workflow verifies those bytes before
publishing them.
...
Temporal Server and the worker sit behind the `scheduled` profile and are not
started by a default `docker compose up`. The profile keeps Temporal Server on
the Compose network and publishes only the Temporal Web UI, bound to
`127.0.0.1:8233`.
...
| - | - | - |
| Bundled with Compose | `docker compose --profile scheduled up -d` starts a Temporal server, its database, and the Temporal Web UI alongside the worker. No configuration. | Evaluation and small self-hosted instances |
...
| - | - |
| Temporal Cloud with `TEMPORAL_API_KEY` | TLS enables automatically; omit `TEMPORAL_TLS`. |
...
| `TEMPORAL_TLS` | Omit when an API key enables TLS automatically; set `true` for TLS without an API key; set `false` for plaintext private networking. |
...
`RANK_CHECK_SCHEDULER_MODE=legacy` is the D1 default. In that mode the worker

...
| `READ_ONLY_DEMO` | Set to `1` only on a separate, sanitized demo installation. Enables one-click access to one existing viewer account, locks account changes, and disables email delivery. Never combine with fixed-OTP flags. |
| `DEMO_MODE` | Set to `editable` only on an isolated demo installation with existing Owner and Viewer accounts. Owner uses ordinary email authentication; public code `000000` admits only Viewer. Never combine with `READ_ONLY_DEMO` or fixed-OTP flags. Saved research stays readable after its 30-day freshness window; only Owner can explicitly request new results. |
| `DEMO_OWNER_ID` | Public ID of the verified Owner in editable mode, distinct from `DEMO_USER_ID`. Owner must own the configured project. Owner changes are visible to demo visitors; do not store private data in this project. |
| `DEMO_USER_ID` | Public ID of the demo's verified, non-admin viewer. It must belong to exactly the configured demo project. Demo sessions expire after two hours. |
| `DEMO_PROJECT_ID` | Public ID of the sole project accessible through the demo identity. Project writes and paid checks are denied by the existing viewer role. |
...
| Variable | Purpose |
| - | - |
| `SCHEDULER_DRIVER` | Selects how this process handles scheduled work. An absent value preserves legacy installs. This is independent from `RANK_CHECK_SCHEDULER_MODE`. |
...
| `TEMPORAL_CONNECT_TIMEOUT_MS` | Temporal client connection timeout in milliseconds. Defaults to `2000`; valid values are integers from `100` through `30000`. |
| `TEMPORAL_CONNECT_FAILURE_COOLDOWN_MS` | Delay in milliseconds before retrying a failed Temporal client connection. Defaults to `30000`; valid values are integers from `100` through `30000`. |
...
| `WORKER_INTENT_POLL_INTERVAL_MS` | Fallback interval for durable user-intent pickup when the realtime wake is unavailable. Defaults to `2000`; valid values are integers from `100` through `60000`. |

### Scheduler driver values

| Value | Meaning |
|-|-|
| `temporal` | this process owns the scheduling engine and may connect to it; the worker uses this |
| `worker` | a separate worker process owns the engine; this process only records what it wants done |
| `none` | no worker exists; scheduled execution is disabled and checks run inline |
...
| `RANK_CHECK_RECONCILER_INTERVAL`, `RANK_CHECK_DISPATCHER_INTERVAL`, `RANK_CHECK_PLANNER_INTERVAL`, `STALE_CHECKS_INTERVAL` | Worker-owned legacy reconciliation, dispatcher polling, planner planning, and stale-check tuning. |
...
| `NEXT_PUBLIC_POSTHOG_KEY`, `NEXT_PUBLIC_POSTHOG_HOST` | Optional consent-gated product analytics and setup replays. Set the key and the EU Cloud host to enable it; leave the key unset to disable the provider and consent banner. See [Product analytics](/self-hosting/analytics). |
...
| `PROVIDER_AVAILABILITY_TTL_SECONDS` | Cache TTL (seconds) for provider availability and balance checks. Defaults to `900` (15m). |
...
| `DOMAIN_OVERVIEW_CACHE_TTL_SECONDS` | Cache TTL (seconds) for domain-overview results in Redis and stored snapshots. Defaults to `43200` (12h). With `READ_ONLY_DEMO=1`, results remain valid for 30 days from retrieval, including existing snapshots, regardless of this setting. |
...
| Variable | Purpose |
| - | - |
| `SEARCH_INSIGHTS_OVERLAP_WORK_MEM_MB` | Per-query overlap sort memory in MiB. Defaults to `16`; valid values are integers from `4` through `32`. |
| `SEARCH_INSIGHTS_TIMING_RECORD_RATE` | Fraction from `0` through `1` of Search Insights renders that emit phase timing logs. Defaults to `0.05`. |
| `SEARCH_SYNC_IMPORT_MONTHS` | Default historical import depth. Valid values are `16`, `12`, `6`, and `3`; invalid values fall back to `16`. |
| `SEARCH_SYNC_PACE` | Default import pace. Use `normal` or `gentle`; gentle uses half the request throughput. Invalid values fall back to `normal`. Project settings override this default. |

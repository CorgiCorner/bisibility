...
Use `/api/v1/liveness` for restarts and `/api/v1/readiness` for traffic.
Anonymous probes expose aggregate status only. An API credential or
`INTERNAL_PROBE_TOKEN` unlocks full `/api/v1/health` diagnostics. Worker or
Temporal failures can return `503` while web reads continue. The source-build
argument `APP_REVISION` identifies the public commit in app and worker reports.
...
6. **Client IP.** Add `BISIBILITY_CLIENT_IP_HEADER` if a proxy or CDN sits in
   front of the app. See [Client IP behind a proxy](/self-hosting/operations#client-ip-behind-a-proxy).
...
Add the worker and Temporal overlays to the first two commands when scheduled
checks are enabled. Use an API credential or `INTERNAL_PROBE_TOKEN` to request
full `/api/v1/health` diagnostics without publishing the token in logs.

| Symptom | Check | Resolution |
| - | - | - |
| App never starts | `db-migrations` exited nonzero | Read its first migration error, fix database connectivity or the reported migration issue, then rerun the same Compose command. Completed migration steps are skipped. |
| Liveness succeeds but readiness returns `503` | Database, blocking migrations, or invalid scheduler configuration | Inspect authenticated `/api/v1/health`; do not route traffic until the failing required component is healthy. |
| Sign-in redirects to the wrong origin | `SITE_URL` and `BETTER_AUTH_URL` | Set both to the exact public origin, including a non-default port, then recreate the app container. |
| Manual checks work but schedules do not | Worker and Temporal overlays, shared namespace, and task queues | Confirm both overlays are present and web and worker use the same Temporal settings. |
| Worker repeatedly restarts | Temporal address, TLS mode, namespace, and database access | Follow [worker startup troubleshooting](/self-hosting/temporal#worker-startup-troubleshooting); the worker does not start Temporal Server itself. |
| Anonymous API callers unexpectedly share one rate-limit bucket | Trusted proxy header configuration | Configure [client IP behind a proxy](/self-hosting/operations#client-ip-behind-a-proxy) or enforce anonymous limits at the edge. |
...
The default stack publishes only the app on `3000`. PostgreSQL and Valkey are
reachable only as `postgres` and `redis` inside the Compose network.

Core does not start the Temporal or worker overlays. The Temporal server stays
inside the Compose network. Its optional `temporal-ui` profile exposes only the
Web UI at `127.0.0.1:8233`.

...
keywords from the current default market to the new market. The legacy
`country` and `city` selectors still resolve to the same location, but they are
deprecated: send `location_key` instead. They are accepted until Bisibility
0.24.0, then removed. Omitting market fields updates only the schedule and does
not move keywords.
...
Deletes the scoped project and returns the deleted project resource. Only an
owner's PAT can delete a project; project API keys receive 403.

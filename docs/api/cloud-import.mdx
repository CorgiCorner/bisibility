...
The endpoint accepts only a direct v5 package. Version 5 accepts only snake-case
package fields, exact lowercase public IDs, and native JSON number and boolean
types. It rejects wrapper objects, body tokens, unknown fields, and v1 through
v4 packages.
...
<Warning>
  Version 4 carries history in each keyword's `rankingHistory` array. Top-level
  `rank_checks` and text-based matching are not supported.
</Warning>
...
Every v5 package includes every section, including empty arrays:
...
`version` must be `4`. `chunk_count` must be between `1` and `500`.
...
```json
{
  "checksum": "sha256:d7070ee876f996d00e1254519e2bf233ea64ab2caebef347513c973e15870d05",
  "kind": "keywords",
...
```json
{
  "checksum": "sha256:7b47a34cd8d581d8a5830ada4937cdf4993f7ac1e4bd3dd8fc9f94a4b69cf935",
  "kind": "sections",
  "sections": {
...
The checksum is the v5 canonical chunk-content identity for idempotent retries. Retrying the
...
`sha256:` value over the canonical JSON body `{ version: 5, ...chunk }`, excluding

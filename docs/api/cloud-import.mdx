...
The endpoint accepts strict v7 packages and compatible v6 packages. Version 7
exports identify every keyword location with `location_key`; the human-readable
...
| `location` | Required human-readable source location label. |
| `location_key` | Required in version 7. Canonical country, region, or city key, optionally qualified with `@language`. It takes precedence over `location`. |
...
<Warning>
  ...
  ranking data. Update the source instance and create a version 7 export.
</Warning>

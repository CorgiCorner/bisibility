...
| `filter[location_key]` or `location_key` | Matches the exact canonical location key, optionally qualified with `@language`. |
| `filter[country]` or `country` | Matches canonical country names and country-name aliases case-insensitively. Deprecated: send `location_key` instead. Accepted until Bisibility 0.24.0, then removed. |
...
| `location_key` | Primary location reference: canonical country, region, or city key, optionally qualified with `@language`, such as `ES/Andalusia/Malaga@en`. Defaults to the project's default market. Takes precedence over the deprecated `country`, `language`, `location`, and `city` fields. |
| `country` | Legacy supported SERP country name. Deprecated: send `location_key` instead. Accepted until Bisibility 0.24.0, then removed. |
| `city` | Legacy city name resolved against `country`; unresolved cities fall back to country-level tracking with a warning. Deprecated: send `location_key` instead. Accepted until Bisibility 0.24.0, then removed. |
| `location` | Legacy alias for `country`; when both are sent, `location` wins. Deprecated: send `location_key` instead. Accepted until Bisibility 0.24.0, then removed. |
...
| Field | Description |
| - | - |
| `keyword` | Identity field. Must match the stored keyword text. |
| `target_url` | Absolute URL, path, `null`, or omitted. |
| `topic` | Topic label, 1 to 80 characters, `null`, or omitted. |
| `intent` | Intent label, 1 to 80 characters, `null`, or omitted. |
| `device` | Identity field. Must match the stored `desktop` or `mobile` device. |
| `location_key` | Identity field and primary location reference. Canonical country, region, or city key, optionally qualified with `@language`; must resolve to the stored market. |
| `country` | Identity field. Legacy country name that must resolve to the stored SERP market. Deprecated: send `location_key` instead. Accepted until Bisibility 0.24.0, then removed. |
| `city` | Identity field. Legacy city name that must resolve to the stored city in the selected country. Deprecated: send `location_key` instead. Accepted until Bisibility 0.24.0, then removed. |
| `location` | Identity field and legacy alias for `country`; when both are sent, `location` wins. Deprecated: send `location_key` instead. Accepted until Bisibility 0.24.0, then removed. |

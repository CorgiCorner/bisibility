...
| `country` | Identity field. Must resolve to the stored SERP market. |
| `city` | Identity field. Must resolve to the stored city in the selected country. |
| `location_key` | Identity field. Canonical country, region, or city key, optionally qualified with `@language`; must resolve to the stored market. |
| `location` | Identity field and alias for `country`; prefer `country`. When both are sent, `location` wins and must resolve to the stored market. |

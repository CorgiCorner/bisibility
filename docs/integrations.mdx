...
| Analytics sources | Traffic and index context from Google Search Console, GA4, or Plausible. | Determining rank positions. |
...
Use the API password generated separately in the DataForSEO dashboard, not your
account password. Open [API Access](https://app.dataforseo.com/api-access) in
the dashboard to find your API login and automatically generated API password.
...
Find your API key in the [SerpApi dashboard](https://serpapi.com/manage-api-key).
...
Provider spend is bounded by a monthly budget set per project in
**Settings > Provider usage**. New projects start at `$50.00`. Project
...
research, and keyword metrics pause once the project reaches the cap, and
...
The project limit controls how many target URLs that project may inspect per
day. It accepts up to 1,000 inspections, leaving room below Google's
2,000-per-day property quota for other tools and retries. Google's own `429`
response remains the runtime backstop and applies the existing account cooldown.
...
| - | - | - |
| Minute rate limit | Stops only the current batch, leaves unchecked URLs untouched, and retries oldest URLs on the next run. | Nobody; it recovers automatically. |
| Google daily quota | Stops the current batch and defers the remaining URLs. Google's daily quota resets around midnight Pacific. | Nobody. If it recurs, lower project limits or reduce the number of projects using the property. |

...
Each completed check can store the raw response from the connected SERP provider
in `rank_checks.raw`. That payload is separate from the rank-history fields that
remain on the row.
...
| Raw SERP provider payload (`rank_checks.raw`) | Kept indefinitely by default. The operator can set `RANK_CHECK_RAW_RETENTION_DAYS` to 1-3650 days or `unlimited`. | Kept for 90 days. |

Raw payload access is a self-hosted feature through direct PostgreSQL access to

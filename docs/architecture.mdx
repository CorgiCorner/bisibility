...
Both processes use PostgreSQL. Redis or Valkey handles rate limits, idempotency,
...
| Redis or Valkey | `lib/redis/` | API and provider rate limiting, idempotency replay, notification pub/sub. |
...
server, which is why `TEMPORAL_ADDRESS`, `TEMPORAL_NAMESPACE`, and
`TEMPORAL_TASK_QUEUE` are required. The web app needs the same address, not to
manage schedules, but to start workflows for manual checks.

The Temporal server is replaceable. The Docker `scheduled` profile bundles one
for convenience, but the same worker image runs unchanged against a self-managed
cluster or Temporal Cloud. See
[Temporal Server or Temporal Cloud](/self-hosting#temporal-server-or-temporal-cloud).
...
| Data | Self-hosted | Cloud |
| - | - | - |
| Rank-check row, `position`, and `organicRanks` | Kept for the lifetime of the workspace. | Kept for the lifetime of the workspace, with no plan-based retention tier. |
...
[Database growth](/self-hosting#database-growth) for worker configuration and
purge behavior.
...
**Redis** backs the cross-process concerns: API and outbound provider rate

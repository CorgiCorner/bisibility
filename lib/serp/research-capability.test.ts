...
describe("research market capability", () => {
  it("uses the generated country-language catalog", () => {
    expect(supportsResearchScope("ES", "es")).toBe(true);
    expect(supportsResearchScope("ES", "en")).toBe(false);
    expect(supportsResearchScope("BE", "fr")).toBe(true);
    expect(supportsResearchScope("NO", "no")).toBe(true);
  ...
  it("owns the country-to-Labs numeric translation used by research consumers", () => {
    ...
    expect(supportsResearchScope("CZ", "cs")).toBe(true);
    expect(researchCountryLocationCode("IS")).toBeNull();
    expect(supportsResearchScope("IS", "is")).toBe(false);

...
export async function getCloudMigrationCompatibility(input: unknown) { const data = parseActionInput(projectInputSchema, input); const actor = await getActionActor(); const project = await requireMigrationProject(actor, data.projectId); const [schema, keywords, rankChecks] = await Promise.all([appliedMigrationSummary(), prisma.keyword.count({ where: { projectId: project.id } }), prisma.rankCheck.count({ where: { keyword: { projectId: project.id }, ...whereExecutedChecks() } })]); const version = packageVersion(); const limits = pushMigrationLimits(); return { appVersion: version.value, appVersionSource: version.source, cloudOrigin: configuredMigrationTargetOrigin(), data: { keywords, rankChecks }, limits: { pushMaxKeywords: limits.maxKeywords, sessionsRequired: shouldUseSessions({ keywords, rankChecks }) }, schema }; }

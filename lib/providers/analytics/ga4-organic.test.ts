...
describe("organic sessions reports", () => {
  ...
  it("normalizes valid rows and follows the offset through a full landing-page page", async () => {
    ...
    await expect(
      ...
      requestedRows: 4,
      rows: [
        { date: "2026-07-01", landingPage: "/pricing", sessions: 8 },
        { date: "2026-07-02", landingPage: "docs", sessions: 6 },
      ],
    });
  ...
  it("discards a surplus response row before continuing at the requested offset", async () => {
    ...
    await expect(
      ...
      pages: 2,
      rows: [
        { date: "2026-07-01", landingPage: "/first", sessions: 1 },
        { date: "2026-07-01", landingPage: "/second", sessions: 2 },
        { date: "2026-07-02", landingPage: "/third", sessions: 3 },
      ],
    });
  ...
  it("reports when a full page stops at the provider row ceiling", async () => {
    ...
    await expect(
      ...
      pages: 1,
      requestedRows: 1,
      rows: [{ date: "2026-07-03", landingPage: "/docs", sessions: 6 }],
    });
  });
});

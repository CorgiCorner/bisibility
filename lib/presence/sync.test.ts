...
describe("presence sync", () => {
  beforeEach(() => {
    ...
    process.env.BISIBILITY_GSC_INSPECTION_DAILY_BUDGET = "";
  ...
  it("shares the property budget across projects and defers the second project locally", async () => {
  ...
  it.each(["daily", "unknown"] as const)(
    "drains the property budget until midnight after a %s-scope provider 429",
    async (scope) => {
      mocks.prisma.project.findMany.mockResolvedValue([{ id: "project_1" }, { id: "project_2" }]);
      mocks.prisma.project.findFirst.mockImplementation(({ where }) => {
        const id = where.OR[0].id;
  ...
  it("does not drain the daily budget after a minute-scope provider 429", async () => {
    mocks.prisma.project.findMany.mockResolvedValue([{ id: "project_1" }, { id: "project_2" }]);
    mocks.prisma.project.findFirst.mockImplementation(({ where }) => {
      const id = where.OR[0].id;
      return Promise.resolve({ defaults: { inspectionDailyLimit: 50 }, id });
